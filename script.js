...
function setupGame() {
    ...
    gameData.forEach((item, index) => {
        ...
        const fixedLetterIndexInAnswer = answer.indexOf(item.letter); 
        
        let wordStartingGridColumn = PIVOT_COLUMN_INDEX - fixedLetterIndexInAnswer;
        ...
        for (let charIndex = 0; charIndex < answer.length; charIndex++) {
            ...
            if (charIndex === fixedLetterIndexInAnswer) {
...
function checkAnswers() {
    ...
    gameData.forEach((item, index) => {
        ...
        sortedCells.forEach(cell => { playerWord += cell.textContent || ''; });

        const centralIndex = item.answer.toUpperCase().indexOf(item.letter);

        const fullPlayerWord = 
            playerWord.substring(0, centralIndex) + 
            item.letter + 
            playerWord.substring(centralIndex);

        if (fullPlayerWord === item.answer.toUpperCase()) {
            rowCells.forEach(cell => { cell.style.backgroundColor = correctBg; }); 
        } else {
